...
{
    QSqlQuery q;
    if (!q.exec("SELECT NVL(MAX(IDLABO), 0) + 1 FROM HICHEM.LABS")) {
...
bool Labs::ajouter(QString *err) const
{
    ...
    QSqlQuery q;
    q.prepare("INSERT INTO HICHEM.LABS "
              "(IDLABO, NOMLABO, RESPONSABLE, NUMERO, LOCALISATION, "
              "SPECIALITE, DISPONIBILITE, RESULTAT, PAIEMENT, IDEMP) "
              "VALUES (:id, :nom, :resp, :num, :loc, :spec, :disp, :res, :paiement, :idemp)");
...
{
    QSqlQuery q;
    q.prepare("UPDATE HICHEM.LABS SET "
...
bool Labs::chargerTout(QVector<Row> &out, QString *err)
{
    ...
    q.prepare("SELECT IDLABO, NOMLABO, RESPONSABLE, NUMERO, LOCALISATION, "
              ...
              "FROM HICHEM.LABS ORDER BY IDLABO DESC");
...
bool Labs::getById(const QString& idLabo, Row &outRow, QString *err)
{
    ...
    q.prepare("SELECT IDLABO, NOMLABO, RESPONSABLE, NUMERO, LOCALISATION, "
              "DISPONIBILITE, SPECIALITE, RESULTAT, PAIEMENT "
              "FROM HICHEM.LABS WHERE IDLABO = :id");
    q.bindValue(":id", idLabo);
