...
void MainWindow::loadEmployees()
{
    ...
    if (!Employe::chargerTout(rows, &errMsg)) {
        QMessageBox::warning(this,"Erreur","Impossible de charger les employés : " + errMsg);
        return;
    }
    ...
    for (const auto &r : rows) {
        ...
        ui->TableEmp->setItem(rowIndex, 0, new QTableWidgetItem(r.cin));
...
{
    qDebug() << ">>> [Ajout Employé] Bouton Enregistrer cliqué !";
    ...
    QString typeContrat = ui->lineTypeContratAdd->text().trimmed();
    ...
    if (ui->dateEmbaucheAdd) {
        dateEmbauche = ui->dateEmbaucheAdd->date();
    ...
    // === VALIDATIONS ===
    if (cin.isEmpty() || cin.length() != 8 || !QRegularExpression("^[0-9]{8}$").match(cin).hasMatch()) {
        QMessageBox::warning(this, "Erreur CIN",
                             "Le CIN doit contenir exactement 8 chiffres (ex: 12345678).");
        ui->lineCINAdd->setFocus();
        return;
    }

    bool existe = Employe::usernameExiste(username);
    if (username.isEmpty() || existe) {
        QMessageBox::warning(this,
                             "Username déjà utilisé",
                             "Ce username existe déjà. Choisissez-en un autre.");
        ui->lineUsernameAdd->setFocus();
        return;
    }
    if (password.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Le mot de passe est obligatoire.");
        ui->linePasswordAdd->setFocus();
        return;
    }
    if (email.isEmpty() || !email.contains("@") || !email.contains(".")) {
        QMessageBox::warning(this, "Erreur", "Email invalide ou manquant.");
    ...
    if (nom.isEmpty()) {
        ...
        ui->lineNomAdd->setFocus();
        return;
    }
    if (prenom.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Le prénom est obligatoire.");
        ui->linePrenomAdd->setFocus();
        return;
    }

    if (salaire <= 0) {
        QMessageBox::warning(this, "Erreur", "Le salaire doit être supérieur à 0.");
        ui->lineSalaireAdd->setFocus();
        return;
    }
    if (role.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Veuillez sélectionner un rôle dans la liste.");
    ...
    if (e.ajouter(&errMsg)) {
        ...
        QMessageBox::critical(this, "Erreur ajout",
                              "Échec de l'ajout :\n" + errMsg);
    }
}
...
void MainWindow::on_btnSupprimer_emp_clicked()
{
    ...
    if (row < 0) {
        QMessageBox::warning(this, "Aucune sélection",
                             "Veuillez sélectionner une ligne dans le tableau avant de supprimer.");
        return;
    }

    QTableWidgetItem *item = ui->TableEmp->item(row, 0);
    if (!item) {
        QMessageBox::critical(this, "Erreur", "Impossible de lire l'ID de la ligne.");
        return;
    }

    QString idEmploye = item->data(Qt::UserRole).toString();
    if (idEmploye.isEmpty()) {
        idEmploye = item->text().trimmed();
        if (idEmploye.isEmpty()) {
            QMessageBox::critical(this, "Erreur", "ID/CIN introuvable pour cette ligne.");
            return;
        }
    }
    ...
    QMessageBox::StandardButton reply = QMessageBox::question(
        ...
        "Voulez-vous vraiment supprimer l'employé :\n" +
            ui->TableEmp->item(row, 1)->text() + " " + ui->TableEmp->item(row, 2)->text() + " ?\n" +
            "(ID : " + idEmploye + ")",
        QMessageBox::Yes | QMessageBox::No
        );
    ...
    QString errMsg;
    if (Employe::supprimer(idEmploye, &errMsg)) {
        QMessageBox::information(this, "Succès",
                                 "L'employé a été supprimé avec succès.");
        loadEmployees();
    } else {
        QMessageBox::critical(this, "Erreur suppression",
