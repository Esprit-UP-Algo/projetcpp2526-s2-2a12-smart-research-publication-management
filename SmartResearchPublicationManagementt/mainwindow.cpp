...
MainWindow::MainWindow(Arduino *keypadArduino, Arduino *rfidArduino, RfidHandler *rfidHandler, const QString &rfidPort, QWidget *parent)
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    A               = keypadArduino;
    m_rfidArduino   = rfidArduino;
    m_rfidHandler   = rfidHandler;
    m_rfidExcludePort = rfidPort;
...
void MainWindow::on_btn_exportt_clicked() {
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
        ...
        out << "body { font-family: 'Segoe UI', Arial, sans-serif; background-color: #f4f7f6; padding: 20px; }";
        out << ".container { background: white; border-radius: 10px; padding: 20px; border: 1px solid #d1d5db; }";
        out << ".main-header { text-align: center; margin-bottom: 20px; border-bottom: 3px solid #1e3a8a; padding-bottom: 10px; }";
        out << ".title { font-size: 20pt; font-weight: bold; color: #1e3a8a; text-transform: uppercase; }";
        out << "table { border-collapse: collapse; width: 100%; }";
        out << "th { background-color: #1e3a8a; color: white; padding: 12px; text-align: center; border: 1px solid #cbd5e1; }";
        out << "td { padding: 10px; border: 1px solid #e2e8f0; text-align: center; color: #334155; }";
        out << "tr:nth-child(even) { background-color: #f9fafb; }";
        out << ".present { color: #008000; font-weight: bold; }";
        out << ".absent { color: #FF0000; font-weight: bold; }";
        out << "</style></head><body>";

        out << "<div class='container'>";
        out << "  <div class='main-header'>";
        out << "    <div class='title'>Rapport Journalier</div>";
        out << "    <div style='font-size: 14pt; color: #4b5563;'>Date : " << dateAffichage << "</div>";
        out << "  </div>";
        ...
        for (int r = 0; r < model->rowCount(); r++) {
            QVariant rawDate = model->index(r, 4).data();
            QDate dateLigne;

            // Conversion intelligente de la date
            if (rawDate.type() == QVariant::DateTime) {
                dateLigne = rawDate.toDateTime().date();
            } else if (rawDate.type() == QVariant::Date) {
                dateLigne = rawDate.toDate();
            } else {
                dateLigne = QDate::fromString(rawDate.toString().left(10), "yyyy-MM-dd");
                if (!dateLigne.isValid())
                    dateLigne = QDate::fromString(rawDate.toString().left(10), "dd/MM/yyyy");
            }

            // FILTRE : On n'affiche que les données d'aujourd'hui

                out << "<tr>";
                for (int c = 0; c < model->columnCount(); c++) {
                    QVariant cellData = model->index(r, c).data();
                    QString val = cellData.toString();
                    QString valLower = val.toLower().trimmed();

                    // AJUSTEMENT : Si c'est la colonne date (4), on formate proprement
                    if (c == 4) {
                        out << "<td>" << dateLigne.toString("dd/MM/yyyy") << "</td>";
                    }
                    else if (valLower == "présent" || valLower == "present") {
                        out << "<td><span class='present'>PRÉSENT</span></td>";
                    }
                    else if (valLower == "absent") {
                        out << "<td><span class='absent'>ABSENT</span></td>";
                    }
                    else {
...
void MainWindow::checkAndShowInventoryAlerts() {
    ...
    // ── Windows OS-level notification (system tray) ───────────────────────
    if (!QSystemTrayIcon::isSystemTrayAvailable()) {
        qDebug() << "[INVENTORY] System tray not available — skipping OS notification.";
        return;
    }
    ...
    if (!m_trayIcon) {
        ...
        // Use the application window icon; fall back to built-in warning icon
        QIcon appIcon = windowIcon();
        if (appIcon.isNull())
            appIcon = style()->standardIcon(QStyle::SP_MessageBoxWarning);
        m_trayIcon->setIcon(appIcon);
        m_trayIcon->setToolTip("Smart Research Lab — Inventory Alerts");
        m_trayIcon->show();
    }

    // Build a plain-text summary for the Windows toast notification
    QString body;
    for (const auto &row : std::as_const(alerts)) {
        const QString label = row.name.isEmpty() ? row.sku : row.name;
        body += QString("%1  |  Disponible: %2  |  Seuil: %3\n")
                    .arg(label)
                    .arg(row.qtAv)
                    .arg(row.threshold);
    }

    // Show the Windows balloon / toast notification
    m_trayIcon->showMessage(
        QString("⚠ Alertes Inventaire — %1 produit(s)").arg(alerts.size()),
        body,
...
{
    // Si le keypad Arduino n'est pas connecté, boucle de connexion avec réessai
    if (!A || !A->getserial() || !A->getserial()->isOpen()) {
        bool connecte = false;
        while (!connecte) {
            // Dialogue : brancher et scanner
            QDialog dlg(this);
            dlg.setWindowTitle("Connexion Arduino Keypad");
            dlg.setFixedWidth(420);

            auto *layout  = new QVBoxLayout(&dlg);
            auto *label   = new QLabel("🔌 Branchez l'Arduino Keypad via USB\npuis cliquez sur Connecter.", &dlg);
            label->setStyleSheet("font-size:14px; padding:8px;");
            label->setAlignment(Qt::AlignCenter);

            auto *statusLabel = new QLabel("", &dlg);
            statusLabel->setAlignment(Qt::AlignCenter);
            statusLabel->setStyleSheet("font-size:13px; padding:4px;");

            auto *btnConnect = new QPushButton("🔗 Connecter", &dlg);
            auto *btnIgnorer = new QPushButton("Ignorer", &dlg);
            btnConnect->setMinimumHeight(36);
            btnIgnorer->setMinimumHeight(36);

            auto *btnLayout = new QHBoxLayout;
            btnLayout->addWidget(btnConnect);
            btnLayout->addWidget(btnIgnorer);
            ...
            bool ignore = false;
            connect(btnIgnorer, &QPushButton::clicked, &dlg, [&]() {
                ignore = true;
                dlg.accept();
            });

            connect(btnConnect, &QPushButton::clicked, &dlg, [&]() {
                statusLabel->setText("⏳ Recherche en cours...");
                statusLabel->setStyleSheet("font-size:13px; color:#f59e0b; padding:4px;");
                QApplication::processEvents();

                // Refermer l'ancien port si nécessaire
                if (A->getserial()->isOpen())
                    A->getserial()->close();

                // Exclure le port RFID seulement s'il est encore actif
                QString excludePort = "";
                if (m_rfidArduino && m_rfidArduino->getserial() && m_rfidArduino->getserial()->isOpen())
                    excludePort = m_rfidExcludePort;

                int ret = A->connect_arduino(excludePort);
                if (ret == 0) {
                    if (m_rfidHandler)
                        m_rfidHandler->reconnectKeypad(A);
                    statusLabel->setText("✅ Connecté sur " + A->getarduino_port_name());
                    statusLabel->setStyleSheet("font-size:13px; color:#22c55e; padding:4px;");
                    QApplication::processEvents();
                    connecte = true;
                    QTimer::singleShot(800, &dlg, &QDialog::accept);
                } else {
                    statusLabel->setText("❌ Arduino introuvable. Vérifiez le câble USB et réessayez.");
                    statusLabel->setStyleSheet("font-size:13px; color:#ef4444; padding:4px;");
                }
            });

            dlg.exec();

            if (ignore || connecte) break;
        }
    }
...
void MainWindow::on_btnLabReserveValidate_clicked()
{
    ...
    // ── Check threshold alerts after reservation (Windows OS notification) ──
    checkAndShowInventoryAlerts();
}
